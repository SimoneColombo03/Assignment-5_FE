...
% COMPUTE_UPFRONT_DIG_RISK - Calculates the fair upfront X% considering digital risk.
%
% INPUTS:
%   notional            - [Scalar] Total principal amount
%   spot_vols_matrix    - [Matrix] Stripped spot volatilities for each caplet and strike
%   strike_grid         - [Vector] Grid of strikes corresponding to the vol matrix columns
%   spot_vol_parameters - [Struct] Contains fwd_libor, yf_between_caplets, T_expiry, and df_caplets
%   dates_bootstrap     - [Vector] Dates from the discount curve bootstrapping
%   discounts           - [Vector] Discount factors from the bootstrapping
%   start_date          - [Scalar] The start date of the bond
%   maturity_date_unadj - [Scalar] Unadjusted maturity date
%   first_coupon_rate   - [Scalar] The fixed rate for the first quarter
%   mode_after_6y       - [String] 'cap' or 'digital' for the final sub-period payoff
%
% OUTPUT:
%   upfront             - [Scalar] The fair upfront X% (as a decimal)
    ...
    n_total_q = 40; % Total quarterly coupons over 10 years
    q_3y = 12;      % Last coupon of the first 3-year period
    q_6y = 24;      % Last coupon of the second 3-year period

    % --- 2. Strike Volatility Interpolation ---
    % Use spline interpolation to get the spot vol at the specific contractual strikes
    K1 = 0.0420; % Strike for t <= 3y
    K2 = 0.0470; % Strike for 3y < t <= 6y
    
    % Select K3 based on the chosen mode for the last 4 years
    ...
    yf_t1 = yearfrac(start_date, t1, 2); % Act/360
    pv_B = notional * yf_t1 * df_t1 * first_coupon_rate;

    % Q2 to Q40: Optionality included (Caplets and Digitals)
    for k = 2:n_total_q
    ...
    % PV(Floating Leg) = Notional * (Discount_Start - Discount_End) + Notional * Margin * BPV
